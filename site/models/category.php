 ...
 * @since  1.5
 */
class BfdownloadmanagerModelCategory extends JModelList
{
	/**
	 * Category items data
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$value = $this->getUserStateFromRequest('com_bfdownloadmanager.category.filter.' . $itemid . '.tag', 'filter_tag', 0, 'int', false);
		$this->setState('filter.tag', $value);

		// Optional filter text
		$search = $app->getUserStateFromRequest('com_bfdownloadmanager.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
		...
		$listOrder = $app->getUserStateFromRequest('com_bfdownloadmanager.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
		...
		{
			$limit = $app->getUserStateFromRequest('com_bfdownloadmanager.category.list.' . $itemid . '.limit', 'limit', $params->get('display_num'), 'uint');
		}

		$this->setState('list.limit', $limit);

		// Set the depth of the category query based on parameter
		$showSubcategories = $params->get('show_subcategory_bfdownloadmanager', '0');

		if ($showSubcategories)
		{
			$this->setState('filter.max_category_levels', $params->get('show_subcategory_bfdownloadmanager', '1'));
	...
	public function getItems()
	{
		...
		{
			$model = JModelLegacy::getInstance('Downloads', 'BfdownloadmanagerModel', array('ignore_request' => true));
			...
			$model->setState('list.ordering', $this->_buildBfdownloadmanagerOrderBy());
			...
			// Filter.subcategories indicates whether to include downloads from subcategories in the list or blog
	...
	public function getCategory()
	{
		if (!is_object($this->_item))
		{
			...
			$categories = JCategories::getInstance('Bfdownloadmanager', $options);
