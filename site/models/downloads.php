 ...
 * @since  1.6
 */
class BfdownloadmanagerModelDownloads extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = 'ordering', $direction = 'ASC')
	{
		...
		if ((!$params->get('show_noauth')) || (!JComponentHelper::getParams('com_bfdownloadmanager')->get('show_noauth')))
		{
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . serialize($this->getState('filter.download_id'));
		$id .= ':' . $this->getState('filter.download_id.include');
	...
	protected function getListQuery()
	{
		...
		if ($this->getState('filter.frontpage'))
		{
			if ($orderby_sec === 'front')
			{
				...
				$query->join('INNER', '#__bfdownloadmanager_frontpage AS fp ON fp.bfdownloadmanager_id = a.id');
			}
			else
			{
		...
		{
			$query->select('fp.ordering');
			$query->join('LEFT', '#__bfdownloadmanager_frontpage AS fp ON fp.bfdownloadmanager_id = a.id');
		}
		...
		if (JPluginHelper::isEnabled('bfdownloadmanager', 'vote'))
		{
		...
		{
			/**
			 * If category is archived then download has to be published or archived.
			 * Or categogy is published then download has to be archived.
			 */
		...
		// Filter by a single or group of downloads.
	...
	public function getItems()
	{
		...
		{
			$downloadParams = new Registry($item->attribs);
			...
			/**
			 * For blogs, download params override menu item params only if menu param = 'use_download'
			 * Otherwise, menu item params control the layout
			 * If menu item is 'use_download' and there is no download param, use global
			 */
				...
				|| ($this->getState('params')->get('layout_type') === 'blog'))
			{
				...
				{
					if ($value === 'use_download')
					{
						// If the download has a value, use it
						if ($downloadParams->get($key) != '')
						{
							// Get the value from the download
							$downloadArray[$key] = $downloadParams->get($key);
						}
						else
						{
							// Otherwise, use the global value
							$downloadArray[$key] = $globalParams->get($key);
						}
					}
				}

				// Merge the selected download params
				if (count($downloadArray) > 0)
				{
					$downloadParams = new Registry($downloadArray);
					$item->params->merge($downloadParams);
				}
			}
			else
			{
				// For non-blog layouts, merge all of the download params
				$item->params->merge($downloadParams);
			}
			...
			if (!$guest)
			{
				$asset = 'com_bfdownloadmanager.download.' . $item->id;
			...
			if ($this->getState('load_tags', $item->params->get('show_tags', '1')))
			{
				...
				$item->tags->getItemTags('com_bfdownloadmanager.download', $item->id);
			}

			if ($item->params->get('show_associations'))
			{
				$item->associations = BfdownloadmanagerHelperAssociation::displayAssociations($item->id);
			}
		}
