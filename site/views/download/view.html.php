 ...
 * @since  1.5
 */
class BfdownloadmanagerViewDownload extends JViewLegacy
{
	protected $item;
    ...
    switch($this->getLayout()) {
      ...
      default:
		...
		$item->readmore_link = JRoute::_(BfdownloadmanagerHelperRoute::getDownloadRoute($item->slug, $item->catid, $item->language));
		...
		if ($active)
		{
			...
			if (strpos($currentLink, 'view=download') && strpos($currentLink, '&id=' . (string) $item->id))
			{
				...
				// Check for alternative layout of download
				elseif ($layout = $item->params->get('download_layout'))
				{
			...
			else
			{
				...
				if (!$infetch && $layout = $item->params->get('download_layout'))
				{
					$this->setLayout($layout);
				}
			}
		...
		$item->tags->getItemTags('com_bfdownloadmanager.download', $this->item->id);
		...
		$results = $dispatcher->trigger('onBfdownloadmanagerBeforeDisplay', array('com_bfdownloadmanager.download', &$item, &$item->params, $offset));
		$item->event->beforeDisplayBfdownloadmanager = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onBfdownloadmanagerAfterDisplay', array('com_bfdownloadmanager.download', &$item, &$item->params, $offset));
