 ...
 * @since  1.5
 */
class BfdownloadmanagerViewCategory extends JViewCategoryfeed
{
	/**
	...
	protected function reconcileNames($item)
	{
		...
		{
			// Compute the download slug
			$item->slug = $item->alias ? ($item->id . ':' . $item->alias) : $item->id;

			// URL link to download
			$link = JRoute::_(BfdownloadmanagerHelperRoute::getDownloadRoute($item->slug, $item->catid, $item->language));

			$item->description .= '<p class="feed-readmore"><a target="_blank" href ="' . $link . '">' . JText::_('COM_BFDOWNLOADMANAGER_FEED_READMORE') . '</a></p>';
		}
