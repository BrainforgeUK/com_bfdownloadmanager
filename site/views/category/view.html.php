...
/**
 * HTML View class for the Bfdownloadmanager component
 *
 * @since  1.5
 */
class BfdownloadmanagerViewCategory extends JViewCategory
{
	/**
	 ...
	 * @since  3.2
	 */
	protected $defaultPageTitle = 'JGLOBAL_DOWNLOADS';
	...
	public function display($tpl = null)
	{
		...
		foreach ($this->items as $item)
		{
			...
			$results = $dispatcher->trigger('onBfdownloadmanagerAfterTitle', array('com_bfdownloadmanager.category', &$item, &$item->params, 0));
			$item->event->afterDisplayTitle = trim(implode("\n", $results));

			$results = $dispatcher->trigger('onBfdownloadmanagerBeforeDisplay', array('com_bfdownloadmanager.category', &$item, &$item->params, 0));
			$item->event->beforeDisplayBfdownloadmanager = trim(implode("\n", $results));

			$results = $dispatcher->trigger('onBfdownloadmanagerAfterDisplay', array('com_bfdownloadmanager.category', &$item, &$item->params, 0));
			$item->event->afterDisplayBfdownloadmanager = trim(implode("\n", $results));
		}
		...
		// For blog layouts, preprocess the breakdown of leading, intro and linked downloads.
		...
		if ($params->get('layout_type') === 'blog' || $this->getLayout() === 'blog')
		{
			...
			if ($order == 0 && $this->columns > 1)
			{
				...
				$this->intro_items = BfdownloadmanagerHelperQuery::orderDownColumns($this->intro_items, $this->columns);
			}
		}
		...
		if ($menu
			&& $menu->component == 'com_bfdownloadmanager'
	...
	protected function prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] !== 'com_bfdownloadmanager' || $menu->query['view'] === 'download' || $id != $this->category->id))
		{
			...
			while (($menu->query['option'] !== 'com_bfdownloadmanager' || $menu->query['view'] === 'download' || $id != $category->id) && $category->id > 1)
			{
