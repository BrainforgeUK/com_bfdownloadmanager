...
/**
 * Bfdownloadmanager Component Query Helper
 *
 * @since  1.5
 */
class BfdownloadmanagerHelperQuery
{
	/**
	...
	public static function buildVotingQuery($params = null)
	{
		...
		if ($voting)
		{
			...
			$join = ' LEFT JOIN #__bfdownloadmanager_rating AS v ON a.id = v.bfdownloadmanager_id';
		}
		else
		{
			$select = '';
	...
	/**
	 * Method to order the intro downloads array for ordering
	 * down the columns instead of across.
	 * The layout always lays the introtext downloads out across columns.
	 * Array is reordered so that, when downloads are displayed in index order
	 ...
	 * @since   1.6
	 */
	public static function orderDownColumns(&$downloads, $numColumns = 1)
	{
		$count = count($downloads);

		// Just return the same array if there is nothing to change
		if ($numColumns == 1 || !is_array($downloads) || $count <= $numColumns)
		{
			$return = $downloads;
		}
		// We need to re-order the intro downloads array
		else
		{
			...
			// Layout the downloads in column order, skipping empty cells
			$i = 0;
			...
			// Now read the $index back row by row to get downloads in right row/col
			...
			for ($row = 1; ($row <= $maxRows) && ($i < $count); $row++)
			{
				for ($col = 1; ($col <= $numColumns) && ($i < $count); $col++)
				{
					$return[$keys[$i]] = $downloads[$index[$row][$col]];
					$i++;
				}
			}
		}
