...
/**
 * Legacy routing rules class from com_bfdownloadmanager
 *
 * @since       3.6
 * @deprecated  4.0
 */
class BfdownloadmanagerRouterRulesLegacy implements JComponentRouterRulesInterface
{
	/**
	...
	public function build(&$query, &$segments)
	{
		...
		// Check again
		if ($menuItemGiven && isset($menuItem) && $menuItem->component != 'com_bfdownloadmanager')
		{
			$menuItemGiven = false;
			unset($query['Itemid']);
		}
		...
		// Are we dealing with an download or category that is attached to a menu item?
		if ($menuItem !== null
		...
		if ($view == 'category' || $view == 'download')
		{
			if (!$menuItemGiven)
			{
				$segments[] = $view;
	...
	public function parse(&$segments, &$vars)
	{
		...
		/*
		 * If there is only one segment, then it points to either an download or a category.
		 * We test it first to see if it is a category.  If the id and alias match a category,
		 * then we assume it is a category.  If they don't we assume it is an download
		 */
		if ($count == 1)
		{
			// We check to see if an alias is given.  If not, we assume it is an download
			...
			$category = JCategories::getInstance('Bfdownloadmanager')->get($id);
		 ...
		 * last segment has a number prepended, it is an download, otherwise, it is a category.
		 */
		if ((!$advanced))
		{
		...
		if (!$category)
		{
			JError::raiseError(404, JText::_('COM_BFDOWNLOADMANAGER_ERROR_PARENT_CATEGORY_NOT_FOUND'));
