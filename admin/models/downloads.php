...
class BfdownloadmanagerModelDownloads extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (JLanguageAssociations::isEnabled())
		{
			$query->select('COUNT(asso2.id)>1 as association')
				->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_bfdownloadmanager.item'))
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__bfdownloadmanager AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
